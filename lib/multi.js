...
var MultiCache = (function(){
  ...
  MultiCache.prototype.stats = function(options) {
    var statuses = this._useMethods(options, 'stats');
    async.each(statuses, function(sync, cb){
      cb(null, sync());
    }, function(err, statusResult){
      if(err){
        return new MultiError('Unable to retrieve stats from adapters');
      }
      return statusResult;
    });
  };
