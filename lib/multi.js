  ...
  MultiCache.prototype.get = function(keys, options, callback) {
    ...
    if(self._useLocalCache(options)) {
      self.localCache.get(keys, function(err, value){
        ...
        if(!isEmpty(value) || !self._useRemoteCache(options)) {
          return callback(err, value);
        }

        self.remoteCache.get(keys, function(err2, value2){
          if(err2) {
            return callback(err2, value2);
          }
          if(options && options.setLocal && !isEmpty(value2)) {
            self.localCache.set(keys, value2, {localCache: true, remoteCache: false}, function(){
              return callback(err2, value2);
            });
          } else {
            return callback(err2, value2);
...
//ADD EVENT 
  MultiCache.prototype.event = function(event, options, callback) {
    var self = this;
    if(typeof options === 'function') {
      callback = options;
      options = null;
    }
	self.localCache.event(event, function(key, value){
...
module.exports = MultiCacheResult;
